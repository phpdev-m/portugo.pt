	...
	// $date_res= strtotime($date);
	 ...
	 echo ' <p> Delivery For</p>
         <h5 class="restaurant_top_right_text_size" style="font-size:20px !important;">'; 
		  ...
		  if(date('d-m-Y',$delivery_time['date_time'])==$today){ echo 'Today';}
		  if(date('d-m-Y',$delivery_time['date_time'])==$tomorrow){ echo 'Tomorrow';} '</h5>';
                   echo '<p>'.date('H:i',$delivery_time['date_time']).'</p>';
		 }else
		 {
///estudo para entrega  proxima
