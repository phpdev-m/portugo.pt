...
<li><a href="<?php echo base_url('admin/signup_content'); ?>">Manage Signup Content</a>
...
<form action="<?php echo base_url('admin/signup_content/edit/'.$query['signup_content_id']); ?>" method="post" enctype="multipart/form-data">
<label class="control-label">Content Title</label>
<input type="text" class="m-wrap span8" name="title" id="title" value="<?php echo ucfirst($query['content_title']); ?>" />
...
<?php 
  //$this->fckeditor->test();
	...
	$this->fckeditor->Value = stripslashes($query['content_desc']);
...
<label class="control-label">Content Image<span class="required">*</span></label>
										
											<input id="file" class="m-wrap span4" value="" type="file" name="file" />
										
                                        <?php if($query['content_image']!=''){ ?>
                                            <img src="<?php echo base_url(); ?>image_gallery/content_image/<?php echo $query['content_image'];?>" style="height:50px; width:50px;"/>
                                            <?php } ?>
                                            
                                            <input type="hidden" name="file_hidden" value="<?php echo $query['content_image'];?>"/> <br /><br />
...
<div id="myDiv"><?php if(isset($error) && $error!=''){?><br /><div class="error" style="margin:10px 0px;"><div id="close" style="float:right; cursor:pointer;">X</div><?=$error?></div><?php } ?></div> <br />
<div style="height: auto;" id="accordion1-1" class="accordion collapse">
<form action="<?php echo base_url('admin/signup_content/edit_portugoese/'.$query['signup_content_id']); ?>" method="post" enctype="multipart/form-data">
<label class="control-label">Content Title</label>
<input type="text" class="m-wrap span8" name="title" id="title" value="<?php echo ucfirst($query['portugoese_content_tittle']); ?>" />
...
<?php 
  //$this->fckeditor->test();
	...
	$this->fckeditor->Value = stripslashes($query['portugoese_content_desc']);
	$this->fckeditor->Width='94%';
	$this->fckeditor->Height='475';
	$this->fckeditor->create();
  ?>
